...
<template>
  <div class="w-full flex flex-col justify-center select-none font-sans text-slate-100 text-left h-[330px] my-auto px-4">
    ...
    <div class="space-y-4 text-xs md:text-[14px] leading-relaxed max-w-4xl font-normal">
      ...
      <p class="text-sm md:text-[16px] font-bold text-white font-serif leading-snug">
        ChatGPT Work는 ChatGPT에게
        <span class="text-blue-400 underline underline-offset-4 decoration-blue-400 font-bold">실제 업무를 위임하는 방식</span>입니다.
      </p>

      <!-- Paragraph 2: 4-Stage Sequential Highlight & Animated Underlines -->
      <p class="text-slate-300 leading-[1.8] break-keep">
        <!-- 1. Underline 1: 단순한 답변 ~ 짧은 초안 작성 -->
        <span class="relative inline-block font-medium text-slate-100">
        ...
        <!-- 2. Chat Highlight -->
        <span class="hl-brush font-bold text-sky-300 px-1.5 py-0.5 mx-0.5" :class="{ active: currentStage >= 2 }">Chat</span>을 사용합니다. 반면, 기획서, 발표 슬라이드, 데이터 분석, 정기 업데이트처럼
        <!-- 3. Underline 2: 내가 직접 검토하고 ~ 결과물(파일) -->
        <span class="relative inline-block font-bold text-white">
        ...
        <span class="hl-brush font-black text-blue-400 px-1.5 py-0.5 mx-0.5 shadow-2xs" :class="{ active: currentStage >= 4 }">ChatGPT Work</span>를 사용합니다.
      </p>

      <!-- Paragraph 3: File/Tool Orchestration & Human Approval -->
      <p class="text-slate-300 leading-relaxed break-keep">
      ...
      <p class="text-slate-400 leading-relaxed break-keep text-[12.5px] pt-3 border-t border-white/10">
        데스크톱 앱에서는 내 컴퓨터의
...
.hl-brush {
  background-image: linear-gradient(120deg, rgba(250, 204, 21, 0.4) 0%, rgba(234, 179, 8, 0.55) 100%);
