    ...
    <div class="relative w-full grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      >
        <div>
          ...
          <div class="flex items-center justify-between pb-1.5 border-b border-rose-500/30 mb-2">
            <div class="flex items-center gap-1.5 font-bold text-rose-300 text-xs font-serif">
              <FileX :size="15" class="text-rose-400" />
              <span>깜깜이 시험 (기존 순수 LLM)</span>
            </div>
            <span class="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-rose-950/90 text-rose-300 border border-rose-500/40">
              CLOSED-BOOK
            </span>
          </div>

          <!-- Working Process -->
          <div class="space-y-1.5 text-[11.5px] text-slate-200 leading-relaxed">
            ...
            <div class="flex items-start gap-1">
              ...
              <span class="text-rose-300 font-medium">목포시 최신 복지 지침서가 머릿속에 없어 당황</span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          :class="stage >= 2 ? 'border-rose-400/60 bg-rose-950/60' : 'border-white/10'"
        >
          <div class="flex items-center justify-between mb-0.5">
            <span class="text-[9.5px] font-mono font-bold text-slate-300">답변 출력:</span>
            <span
              v-if="stage >= 2"
              class="text-[9px] font-bold text-rose-200 bg-rose-950 px-1.5 py-0.2 rounded border border-rose-400/60 flex items-center gap-1 transition-opacity duration-300"
            >
              <AlertTriangle :size="10" class="text-rose-400" /> 환각 오류 (근거 없음)
            </span>
          </div>
          <p
            class="text-xs font-bold transition-all duration-300 py-0.5"
            :class="stage >= 2
              ? 'text-rose-300 font-mono line-through decoration-rose-400 decoration-[2.5px]'
      ...
      >
        <div>
          ...
          >
            ...
            <div class="text-[11px] text-slate-200 leading-relaxed font-sans">
              <span>...제4조 </span>
              <span
                class="transition-all duration-700 rounded px-1.5 py-0.5 font-bold inline-block"
                :class="stage >= 4
                  ? 'bg-amber-950/80 text-amber-300 border border-amber-400/70 shadow-xs'
