...
const rows: ComparisonRow[] = [
  ...
  },
  {
    ...
    work: '실무 원자료를 종합하여 완성형 사업 문서 제작',
  ...
  {
    category: '실무 대상',
    chat: '본격 작성 전 아이디어 발상 및 초안 구상',
    work: '행정 실무자 및 사회복지사 (본 교육 핵심)',
    ...
    <div class="slide-footer quote-box text-xs mt-1 font-medium relative overflow-hidden min-h-[42px] flex items-center">
      <transition name="quote-fade" mode="out-in">
        <div v-if="!isCodexVisible" key="quote-chat" class="w-full">
          <span class="i-carbon:target text-cyan-300 inline-block mr-1"></span> <strong>핵심 공식</strong>: Chat이 <strong>"생각을 정리하는 대화 상대"</strong>라면, Work는 <strong>"내 컴퓨터의 실무 자료를 직접 읽고 완성형 문서를 만들어주는 전담 비서"</strong>입니다.
        </div>
        <div v-else key="quote-codex" class="w-full text-slate-100 font-medium">
          <span class="i-carbon:flash text-amber-300 inline-block mr-1"></span> <strong class="text-purple-300 font-bold">3대 환경 최종 정리</strong>: 일반 대화는 <strong class="text-sky-300 font-bold">Chat</strong>, 실무 문서 제작은 <strong class="text-blue-400 font-bold">Work</strong>, 대량 데이터 전처리·코딩 자동화는 <strong class="text-purple-300 font-bold">Codex</strong>로 역할이 완벽히 분담됩니다.
        </div>
      </transition>
    </div>
  </div>
